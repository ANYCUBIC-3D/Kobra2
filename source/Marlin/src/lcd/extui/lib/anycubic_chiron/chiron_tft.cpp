      ...
      case 31: { // A31 Adjust all Probe Points
        switch (panel_command[4]) {
          ...
          case 'D':   // Save Z Offset tables and restore leveling state
				...
				const float currval = getMeshPoint(pos);SERIAL_ECHOPAIR("old: ", currval);
				setMeshPoint(pos, constrain(currval + live_Zoffset, AC_LOWEST_MESHPOINT_VAL, 5));
          ...
          case 'G':   // Get current offset
			...
			SERIAL_ECHOLNPAIR("A31V ", getZOffset_mm());
			live_Zoffset = 0;
          break;
          case 'S': { 

		    float Zshift = atof(&panel_command[5]);
		    float currZOffset = getZOffset_mm()+Zshift;
			live_Zoffset +=Zshift;	 
		    setZOffset_mm(currZOffset );
		    SendtoTFT(PSTR("A31V "));
		    TFTSer.println(getZOffset_mm());

        int16_t steps = mmToWholeSteps(Zshift, Z);
        ...
        #endif
        setMeshPoint(pos, constrain(currval +Zshift, AC_LOWEST_MESHPOINT_VAL, 5));
        }
		}
		else
			{
			  setSoftEndstopState(false);
               ...
               setAxisPosition_mm(currZpos+Zshift, Z);

...
namespace Anycubic {
  ...
  void ChironTFT::Startup() {
   ...
   // #endif
    ...
    ExtUI::injectCommands_P(AC_cmnd_enable_leveling);
    ...
    SERIAL_ECHOLNPGM("Lcd Startup finish!");
  }
    ...
    switch (printer_state) {
      case AC_printer_probing: {
        ...
        if (strcmp_P(msg, MARLIN_msg_probing_failed) == 0) {
          ...
          ExtUI::injectCommands_P(PSTR("G1 Z50 F500"));
          SendtoTFTLN(AC_msg_probing_complete);
          printer_state = AC_printer_idle;
          msg_matched = true;
        }
  ...
  void ChironTFT::CheckHeaters() {
    ...
    if (hotbed_state != AC_heater_temp_reached) {
      if (WITHIN(getActualTemp_celsius(BED) - getTargetTemp_celsius(BED), -0.5f, 0.5f)) {
  ...
  void ChironTFT::SelectFile() {
    ...
    #endif
	 SERIAL_ECHOLNPAIR_F(" TFT File: ",selectedfile);
  ...
  void ChironTFT::InjectCommandandWait(PGM_P cmd) {
    //ExtUI::injectCommands_P(cmnd); queue.enqueue_now_P(cmd);
  ...
  void ChironTFT::ProcessPanelRequest() {
    ...
    int8_t req = atoi(&panel_command[1]);
	 //SERIAL_ECHOLNPAIR("PanelRequest->A", req);

    // Information requests A0 - A8 and A33
    if (req <= 8 || req == 33|| req == 35) PanelInfo(req);
  ...
  void ChironTFT::PanelInfo(uint8_t req) {
    ...
    switch (req) {
	  ...
	  case 35: //get power_loss file
	    SERIAL_ECHOLNPGM("********* check power_loss file ***********") ;
        recovery.check();
	  break;
  ...
  void ChironTFT::PanelAction(uint8_t req) {
    switch (req) {
      ...
      case 10: // A10 Resume SD Print
        if (pause_steps == 2||pause_state == AC_paused_idle || printer_state == AC_printer_resuming_from_power_outage)
          resumePrint();
        else
          setUserConfirmed();
        break;
      ...
      case 11:   // A11 Stop SD print
        if (isPrintingFromMedia()) {
          ...
          if (printer_state == AC_printer_resuming_from_power_outage)
            ExtUI::injectCommands_P(PSTR("M1000 C")); // Cancel recovery
          SendtoTFTLN(AC_msg_stop);
          printer_state = AC_printer_idle;
        }
        break;
      ...
      case 14: { // A14 Start Printing
        ...
        //file[0] = '/';
        strcpy(file , selectedfile+1);//file+1 daxiong 2021.3.8
		 SERIAL_ECHOLNPAIR_F(" open File: ",file);
        ExtUI::printFile(file);
        SendtoTFTLN(AC_msg_print_from_sd_card);
		strcpy(recovery.info.display_filename, panel_command + 4);
      } break;

      case 15:   // A15 Resuming from outage
        //if (printer_state == AC_printer_resuming_from_power_outage)
        //  // Need to home here to restore the Z position
         // ExtUI::injectCommands_P(AC_cmnd_power_loss_recovery);
         // ExtUI::injectCommands_P(PSTR("M1000"));  // home and start recovery
		SERIAL_ECHOLN("******** check again the powerloss_file A************");

		    if (panel_command[5] == '1')
          	{
          	  SERIAL_ECHOLN("********start resumr print************");
              //ExtUI::injectCommands_P(AC_cmnd_power_loss_recovery);
              ExtUI::injectCommands_P(PSTR("M1000"));  // home and start recovery
          	}
		    else if (panel_command[5] == '0')
		  	{
		  	  ExtUI::injectCommands_P(PSTR("M1000 C")); // Cancel recovery
			  SERIAL_ECHOLN("********cancel resume infor************");
			  TFTSer.print(PSTR("A15 S"));
              TFTSer.println(recovery.info.display_filename);
		    }
      ...
      case 16: { // A16 Set HotEnd temp  A17 S170
        ...
        switch ((char)panel_command[4]) {
          // Set Temp
          case 'S': case 'C': ExtUI::setTargetTemp_celsius(set_Htemp, E0);
        }
      } break;
      ...
      case 17: { // A17 Set bed temp
        ...
        if (panel_command[4] == 'S')
          ExtUI::setTargetTemp_celsius(set_Btemp, BED);
      } break;

      case 18:   // A18 Set Fan Speed
        if (panel_command[4] == 'S')
          ExtUI::setTargetFan_percent(atof(&panel_command[5]), FAN0);
        break;

      case 19:   // A19 Motors off
        if (!ExtUI::isPrinting()) {
          stepper.disable_all_steppers(); // from marlincore.h
          SendtoTFTLN(AC_msg_ready);
        }
        break;

      case 20:   // A20 Read/write print speed
        if (panel_command[4] == 'S'&&atoi(&panel_command[5])>30)
          ExtUI::setFeedrate_percent(atoi(&panel_command[5]));
        else {
          SendtoTFT(PSTR("A20V "));
          TFTSer.println(ExtUI::getFeedrate_percent());
        }
        break;
      ...
      case 21:   // A21 Home Axis  A21 X
        if (!ExtUI::isPrinting()) {
          switch ((char)panel_command[4]) {
            case 'X': ExtUI::injectCommands_P(PSTR("G28 X")); break;
            case 'Y': ExtUI::injectCommands_P(PSTR("G28 Y")); break;
            case 'Z': ExtUI::injectCommands_P(PSTR("G28 Z")); break;
            case 'C': ExtUI::injectCommands_P(PSTR("G28")); break;
          }
        }
        break;
      ...
      case 22:   // A22 Move Axis  A22 Y +10F3000
        ...
        if (!isPrinting()&&!isMoving()) {
          // setAxisPosition_mm() uses pre defined manual feedrates so ignore the feedrate from the panel
//          setSoftEndstopState(true);  // enable endstops
          float newposition = atof(&panel_command[5]); // A22 X-0.1 F3600
          float newspeed = atof(&panel_command[10])/60;
...
//          #endif
          ...
          switch (panel_command[4]) {
            case 'X': setAxisPosition_mm(getAxisPosition_mm(X) + newposition, X, newspeed); break;
            case 'Y': setAxisPosition_mm(getAxisPosition_mm(Y) + newposition, Y, newspeed); break;
            case 'Z': setAxisPosition_mm(getAxisPosition_mm(Z) + newposition, Z, newspeed); break;
            case 'E':   // The only time we get this command is from the filament load/unload menu
              ...
              if (canMove(E0) && !commandsInQueue())
                  setAxisPosition_mm(getAxisPosition_mm(E0) + newposition, E0, newspeed); 
              break;
                //ExtUI::injectCommands_P(newposition > 0 ? AC_cmnd_manual_load_filament : AC_cmnd_manual_unload_filament);
          }
        }
        break;
      ...
      case 23:   // A23 Preheat PLA
        ...
        if (!isPrinting()) {
          ...
          ExtUI::setTargetTemp_celsius(PREHEAT_1_TEMP_BED, BED);
          ExtUI::setTargetTemp_celsius(PREHEAT_1_TEMP_HOTEND, E0);
          SendtoTFTLN();
          hotbed_state = AC_heater_temp_set;
          hotend_state = AC_heater_temp_set;
        }
      ...
      case 24:   // A24 Preheat ABS
        // Ignore request if printing
        if (!isPrinting()) {
          ExtUI::setTargetTemp_celsius(PREHEAT_2_TEMP_BED, BED);
          ExtUI::setTargetTemp_celsius(PREHEAT_2_TEMP_HOTEND, E0);
          SendtoTFTLN();
          hotbed_state = AC_heater_temp_set;
          hotend_state = AC_heater_temp_set;
        }
      ...
      case 25:   // A25 Cool Down
        // Ignore request if printing
        if (!isPrinting()) {
          ExtUI::setTargetTemp_celsius(0, E0);
          ExtUI::setTargetTemp_celsius(0, BED);
          SendtoTFTLN(AC_msg_ready);
          hotbed_state = AC_heater_off;
          hotend_state = AC_heater_off;
        }
    ...
    switch (req) {
        ...
        if (!ExtUI::isPrinting()) {
          ...
          if ((selectedmeshpoint.x == pos.x) && (selectedmeshpoint.y == pos.y)) {
            ...
            if (ExtUI::isPositionKnown()) {
              ...
              #endif
              // Go up before moving
              ExtUI::setAxisPosition_mm(3.0,Z);

              ExtUI::setAxisPosition_mm(17 + (93 * pos.x), X);
              ExtUI::setAxisPosition_mm(20 + (93 * pos.y), Y);
              ExtUI::setAxisPosition_mm(0.0, Z);
              #if ACDEBUG(AC_INFO)
                SERIAL_ECHOLNPAIR("Current Z: ", ExtUI::getAxisPosition_mm(Z));
              #endif
            }
          }
      ...
      case 30: {  // A30 Auto leveling
          ...
          if (ExtUI::isPrinting())
            ...
            SERIAL_ECHOLNPGM("NOT ALLOW LEVELING");
          	}
          else {
            ExtUI::injectCommands_P(PSTR("G28\nG29"));
            printer_state = AC_printer_probing;
            SendtoTFTLN(AC_msg_start_probing);
			SERIAL_ECHOLNPGM("START LEVELING");
          }
      }  break;
      ...
      case 31: { // A31 Adjust all Probe Points
        switch (panel_command[4]) {
          ...
          case 'D':   // Save Z Offset tables and restore leveling state
				...
				const float currval = ExtUI::getMeshPoint(pos);SERIAL_ECHOPAIR("old: ", currval);
				ExtUI::setMeshPoint(pos, constrain(currval + live_Zoffset, AC_LOWEST_MESHPOINT_VAL, 5));
				SERIAL_ECHOLNPAIR("   new: ",  constrain(currval + live_Zoffset, AC_LOWEST_MESHPOINT_VAL, 5));
			  }
        	}
          ...
          case 'G':   // Get current offset
            SendtoTFT(PSTR("A31V "));
            TFTSer.println(ExtUI::getZOffset_mm());
			SERIAL_ECHOLNPAIR("A31V ", ExtUI::getZOffset_mm());
			live_Zoffset = 0;
          break;
          case 'S': { 

		    float Zshift = atof(&panel_command[5]);
		    float currZOffset = ExtUI::getZOffset_mm()+Zshift;
			live_Zoffset +=Zshift;	 
		    ExtUI::setZOffset_mm(currZOffset );
		    SendtoTFT(PSTR("A31V "));
		    TFTSer.println(ExtUI::getZOffset_mm());

        int16_t steps = ExtUI::mmToWholeSteps(Zshift, Z);
        ExtUI::babystepAxis_steps(steps, Z);
		if (isPrinting()){

        GRID_LOOP(x, y) {
        const xy_uint8_t pos={ x, y };
        const float currval = ExtUI::getMeshPoint(pos);
        #if ACDEBUG(AC_MARLIN)
        SERIAL_ECHOLNPAIR("x: ", x, " y: ", y, " z: ", currval);
        #endif
        ExtUI::setMeshPoint(pos, constrain(currval +Zshift, AC_LOWEST_MESHPOINT_VAL, 5));
        }
		}
		else
			{
			  ExtUI::setSoftEndstopState(false);
               if (ExtUI::isAxisPositionKnown(Z)) {
               const float currZpos = ExtUI::getAxisPosition_mm(Z);
			   SERIAL_ECHOLNPAIR("Z: ", currZpos+Zshift);
               ExtUI::setAxisPosition_mm(currZpos+Zshift, Z);
      ...
      case 34: {  // A34 Adjust single mesh point A34 C/S X1 Y1 V123
        if (panel_command[3] == 'C') { // Restore original offsets
          ...
          float currmesh = ExtUI::getMeshPoint(pos);
          ...
          if (printer_state == AC_printer_idle || printer_state == AC_printer_probing /*!isPrinting()*/) {
            ...
            if (selectedmeshpoint.x == pos.x && selectedmeshpoint.y == pos.y) {
              ExtUI::setSoftEndstopState(false);
              float currZpos = ExtUI::getAxisPosition_mm(Z);
              #if ACDEBUG(AC_INFO)
                SERIAL_ECHOLNPAIR("Move Z pos from ", currZpos, " to ", currZpos + constrain(newval - currmesh, -0.05, 0.05));
              #endif
              ExtUI::setAxisPosition_mm(currZpos + constrain(newval - currmesh, -0.05f, 0.05f), Z);
            }
          }
        }
      }  break;

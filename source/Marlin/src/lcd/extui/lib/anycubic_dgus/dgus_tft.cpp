...
namespace Anycubic {
  ...
  lcd_info_t       DgusTFT::lcd_info_back;  // back for changing on lcd, to save flash lifecycle
	uint8_t          DgusTFT::print_speed_mode;
	float            DgusTFT::temp_zoffset;
	float            DgusTFT::move_dis = 1.0;
	bool             DgusTFT::fan_change = false;
	bool             DgusTFT::nozzle_heated_state = false;
	bool             DgusTFT::bed_heated_state = false;
	bool             DgusTFT::preheat_state = false;
    int16_t          DgusTFT::temp_nozzle_Target;
	int16_t          DgusTFT::temp_bed_Target;
	bool             DgusTFT::have_heated_task = false;
	uint8_t          DgusTFT::heated_count = 0;
    bool              DgusTFT::is_bedHeated = false;
  DgusTFT Dgus;
...
//    SendtoTFTLN(AC_msg_ready);
  ...
  void DgusTFT::IdleLoop()  {
    ...
    if(page_index_now == 115) {
      ...
      page176_handle();
    } else if(187 <= page_index_now && page_index_now <= 188) {
      ...
      page203_handle();
		...
		else if( page_index_now == 216 || page_index_now == 218){
			page216_to_219_handle();
		}
		else if( page_index_now == 220 || page_index_now == 222){
			page220_to_223_handle();
		}
		else if( page_index_now == 224 || page_index_now == 225){
			page224_to_225_handle();
		}
		else if (page_index_now == 226 || page_index_now == 227)
		{
			page226_to_227_handle();
		}
		else if(230 <= page_index_now && page_index_now <= 235){
			page228_to_230_handle();
		}
		else if(236 <= page_index_now && page_index_now <= 237){
			page236_to_237_handle();
		}
		else if(238 <= page_index_now && page_index_now <= 241){
			page_238_to_241_handle();
		}
		else {
      ...
      case AC_printer_probing: {
        ...
        if (strcmp_P(msg + strlen(CUSTOM_MACHINE_NAME), MARLIN_msg_ready) == 0) {
          if(probe_cnt == GRID_MAX_POINTS_X*GRID_MAX_POINTS_Y) {
            ...
            //injectCommands_P(PSTR("M500"));
							...
							ChangePageOfTFT(PAGE_CHS_LEVEING);
						#else
        ...
        if (strcmp_P(msg, MARLIN_msg_probing_failed) == 0) {
		  ...
		  if(autoProbe.LeveingFailSattue){
				ChangePageOfTFT(PAGE_CHS_LEVEING_FAILED);	
		 }
...
//          SendtoTFTLN(AC_msg_probing_complete);
        ...
        if (strcmp_P(msg, MARLIN_msg_probe_preheat_start) == 0) {
					#if ENABLED(LEVEING_CALIBRATION_MODULE)	
//						autoProbe.write_sdcard_log("StatusChange::Probe preheat start");
					#else
        ...
        if (strcmp_P(msg, MARLIN_msg_probe_preheat_stop) == 0) {
					#if ENABLED(LEVEING_CALIBRATION_MODULE)
//						autoProbe.write_sdcard_log("StatusChange::Probe preheat stop");
					#else
...
//  	autoProbe.write_sdcard_log("HomingStart");
    if(!isPrintingFromMedia()) {

			//SERIAL_ECHOLNPAIR("autoProbe.need_home_leveing:",autoProbe.need_home_leveing);
  ...
  void DgusTFT::HomingComplete() {
	// autoProbe.write_sdcard_log("HomingComplete");
    ...
    if(lcd_info.language == ExtUI::CHS) {
        ...
        if(PAGE_OUTAGE_RECOVERY == page_index) {
            ...
            data_temp = page_index+3;
				...
				else if(PAGE_CHS_LEVEING <= page_index && page_index <= PAGE_ENG_MODULE_CALIBRATION_DARK){
						data_temp = page_index+2;
				}
				else if(PAGE_CHS_LEVEING_TIP <= page_index && page_index <= PAGE_ENG_LEVEING_FAILED){
						data_temp = page_index +1;
				} 
				else if(PAGE_CHS_SPEED_SILENT<= page_index && page_index <= PAGE_ENG_SPEED_SILENT){
						data_temp = page_index +3;
				}
				else if(PAGE_CHS_RESET <= page_index && page_index <= PAGE_ENG_RESET){
					   data_temp = page_index + 1;
				}
				else if(PAGE_CHS_FAN_ON <= page_index && page_index <= PAGE_CHS_FAN_OFF){
					   data_temp = page_index + 2;
				}
				else {
        ...
        if((control_index&0xF000) == KEY_ADDRESS)// is KEY
        {
          ...
          *p_u8 = data_buf[4];
          ...
          temp=constrain((uint16_t)control_value, 0, HEATER_0_MAXTEMP);
					if(printingIsActive() || temp == 0){
						setTargetTemp_celsius(temp, E0);
						temp_nozzle_Target = temp;
					}
					else {
						nozzle_heated_state = true;
						temp_nozzle_Target = temp;
					}
            ...
            p_u8 =  (unsigned char *)(&control_value) ;//get value
					 ...
					 *p_u8 = data_buf[4];
						...
						if(printingIsActive() || temp == 0){
							...
							temp_bed_Target = temp;
						}
						else{
							bed_heated_state = true;
							temp_bed_Target = temp;
						}
  	    }
          ...
          setTargetFan_percent(temp, FAN0);
  		  ...
  		  *p_u8 = data_buf[4];
					...
					float feedrate = constrain((uint16_t)control_value,5,120);
//  		   feedrate_percentage=constrain(control_value,40,150);
  	    ...
  	    {
  	       static bool resume_status = true;
           p_u8 =  (unsigned char *)(&control_value) ;//get  value
           *p_u8 = data_buf[5];
           p_u8++;
           *p_u8 = data_buf[4];
           ...
           *p_u8 = data_buf[3];
           if((control_value&0x00FFFFFF) == 0x010072) { // startup last gif
             ...
             if(AC_printer_resuming_from_power_outage == printer_state) {
			 	if(resume_status){
				SERIAL_ECHOLNPGM("REG_LCD_READY");
				resume_status = false;
                char filename[64] = { '\0' };
...
//              SERIAL_ECHOLNPAIR("start resumeing from power outage: ", AC_cmnd_power_loss_recovery);
	  ...
	  case 6:   // start print
          if(lcd_txtbox_index > 0 && lcd_txtbox_index  < 6) {   // 1~5
			// autoProbe.SetPrintingHomeStatus(true);
			autoProbe.need_close_fan = true;
		    current_position.z = 0;
			preheat_state = have_heated_task = false;
            if(filenavigator.filelist.seek(lcd_txtbox_page*5+(lcd_txtbox_index-1))) {
    ...
    void DgusTFT::page3_handle(void)
    {
        ...
        switch (key_value)
        {
          ...
          case 2: // resume print
            #if ACDEBUG(AC_MARLIN)
              SERIAL_ECHOLNPAIR("resume print");
              SERIAL_ECHOLNPAIR("printer_state: ", printer_state);
              SERIAL_ECHOLNPAIR("pause_state: ", pause_state);
            #endif
          ...
          case 3: // print stop
            if(isPrintingFromMedia()) {
							// autoProbe.SetPrintingHomeStatus(false);
              ChangePageOfTFT(PAGE_STOP_CONF);
            }
             break;

          case 4: // print change param
					temp_zoffset =  probe.offset.z;
					if(thermalManager.fan_speed[0]){
						fan_change = true;
						SendValueToTFT(fan_change, ADDRESS_PRINT_SETTING_LED_STATUS);
					}	
					else{
						fan_change = false;
						SendValueToTFT(fan_change, ADDRESS_PRINT_SETTING_LED_STATUS);
					}
					//ChangePageOfTFT(PAGE_CHS_FAN_ON);
...
//					ChangePageOfTFT(PAGE_CHS_FAN_ON);
    ...
    void DgusTFT::page5_handle(void)          // print settings
    {
      ...
      static uint16_t fan_speed_last = 0;
      static uint16_t e_temp_last = 0;
      static uint16_t bed_temp_last = 0;
      static millis_t flash_time = 0;
      ...
      break;
        ...
        case 6:
  		  ChangePageOfTFT(PAGE_CHS_SPEED_SILENT + recovery.info.print_mode);
          break;
        ...
        case 7:
          ...
          if(z_change) {
			...
			if(planner.leveling_active){
				autoProbe.calibration_positon.z += probe.offset.z - temp_zoffset;
		...
		if(fan_speed_last != (uint16_t)getActualFan_percent(FAN0)) {
          		fan_speed_last = (uint16_t)getActualFan_percent(FAN0);
				SendValueToTFT((uint16_t)getActualFan_percent(FAN0), TXT_FAN_SPEED_TARGET);

       }
		
	   if(e_temp_last != getTargetTemp_celsius(E0)){
	   		  e_temp_last = getTargetTemp_celsius(E0);
		      SendValueToTFT((uint16_t)getTargetTemp_celsius(E0), TXT_ADJUST_HOTEND);
	   }

	   	if(bed_temp_last != getTargetTemp_celsius(BED)){
		  bed_temp_last = getTargetTemp_celsius(BED);
          SendValueToTFT((uint16_t)getTargetTemp_celsius(BED), TXT_ADJUST_BED);
	   }
        ...
        switch (key_value)
        {
          ...
          case 5:   // turn off the xyz motor
            if(!isMoving()) {
                ...
                set_all_unhomed();
								recovery.file.writeln_P("page7_handle::Disable stepper motors");
            }
          break;
    ...
    void DgusTFT::page8_handle(void)
    {
     ...
     if(key_value == 2 || key_value == 4 ||
        ...
        key_value == 10 || key_value == 12 && !ExtUI::isMoving()) {
         ...
         break;
          ...
          case 6: // cooling
            ...
            setTargetTemp_celsius(0, BED);
						//clean_heated_state();
			nozzle_heated_state = false;
			bed_heated_state = false; 
			is_bedHeated = false;
            ChangePageOfTFT(PAGE_TOOL);
          break;
        ...
        switch (key_value)
        {
          ...
          case 5:   // about
            ...
            SendTxtToTFT(TECH_SUPPORT, TXT_ABOUT_TECH_SUPPORT); 
						SendTxtToTFT(DWIN_UI_VERSION, TXT_ABOUT_UI_VERSION);
...
//                      setAxisPosition_mm(10.0, Z, 5);
        ...
        void DgusTFT::page17_handle(void)
        {
            ...
            switch (key_value)
            {
              ...
              case 2:  // -
              {
                z_off = ExtUI::getZOffset_mm();

                if(z_off <= -5) {
                    return ;
                }

                steps = ExtUI::mmToWholeSteps(-0.05, Z);
                ExtUI::babystepAxis_steps(steps, Z);

                z_off -= 0.05f;
				if(z_off <= -5) z_off = -5;
                ExtUI::setZOffset_mm(z_off);
              ...
              case 3: // +
              {
                ...
                sprintf(str_buf, "%.2f", ExtUI::getZOffset_mm());
                SendTxtToTFT(str_buf, TXT_LEVEL_OFFSET);
        ...
        void DgusTFT::page19_handle(void)   // Filament
        {
            ...
            static bool fil_need_out = 0;
            static bool need_home = 0;
            ...
            switch (key_value)
            {
              ...
              case 2:   // Filament in
              if(getActualTemp_celsius(E0)<205) {
                filament_cmd = FILA_NO_ACT;
                ChangePageOfTFT(PAGE_FILAMENT_HEAT);
              } else {
                if(getTargetTemp_celsius(E0) < 215) {
                  setTargetTemp_celsius(215, E0);
                }
            ...
            if(!isPrinting()) {
              ...
              if(filament_cmd==FILA_IN) {
                ...
                if(fil_need_out && canMove(E0) && !commandsInQueue()) {
									fil_need_out = false;
			  ...
			  if(filament_cmd == FILA_IN ){
			  	...
			  	if(need_home ){
						...
						if(!axis_was_homed(X_AXIS) || !axis_was_homed(Y_AXIS)){						
							injectCommands_P("G28XY");
							queue.process_injected_command_P();
							do_blocking_move_to_xy(X_BED_SIZE-30,Y_MIN_POS,100);//
						}
						else
						{
...
//                      injectCommands_P(PSTR("M108"));
        ...
        void DgusTFT::page33_handle(void)
        {
          ...
          switch (key_value)
          {
            ...
            case 1:   // auto leveling start
            {
              ...
              #endif
			  				// gcode.process_subcommands_now("M2003 F");
							  //autoProbe.wait_for_bed = true;
			  				autoProbe.start_leveing = true;
			  				recovery.open(false);
							if(current_position.z <15) 
							do_blocking_move_to_z(15,5);
							probe.offset.z = 0;
              //injectCommands_P(PSTR("M851 Z0\nG28\nG92.9 Z0\nG29 J\nG29"));
							queue.inject_P(PSTR("G28\n G29"));
              				printer_state = AC_printer_probing;
							#if ENABLED(LEVEING_CALIBRATION_MODULE)
								autoProbe.LeveingTipStatus = 0x01;
								SendTxtToTFT(TXT_RUN, TXT_LEVEING_HEATING);
								SendTxtToTFT(TXT_WAIT, TXT_LEVEING_CLEAN);
								SendTxtToTFT(TXT_WAIT, TXT_LEVEING_PROBE);
								ChangePageOfTFT(PAGE_CHS_LEVEING_TIP);
							#else
								ChangePageOfTFT(PAGE_LEVELING);
							#endif
            }
        ...
        void DgusTFT::page34_handle(void)
        {
          ...
          if(pop_up_index==25)
          {
            pop_up_index=100;
						#if ENABLED(LEVEING_CALIBRATION_MODULE)
							ChangePageOfTFT(PAGE_CHS_MODULE_CALIBRATION);
						#else
							ChangePageOfTFT(PAGE_PreLEVEL);
						#endif
          }
        ...
        switch (key_value)
        {
          ...
          case 1:   // resume
          {
            ...
            SendTxtToTFT(filename, TXT_OUTAGE_RECOVERY_FILE);
						...
						setFeedrate_percent(recovery.info.feedrate_percentage);
              ...
              if(lcd_info.language == ExtUI::ENG) {
                if(page_index_last > 120) {
                	...
                	ChangePageOfTFT(page_index_last);
						...
						recovery.file.writeln_P("page177_to_198_handle::Disable stepper motors");
            stepper.disable_all_steppers();
    ...
    void DgusTFT::page203_handle(void)    // probe precheck failed
    {
		...
		void DgusTFT::page216_to_219_handle(void)
		{
        ...
        case 2:   // 
            ...
            break;
				case 4:   // 
						...
						else if( !axis_is_trusted(X_AXIS) && !axis_is_trusted(Y_AXIS)){
							move_dis = 1.0f;
							SendValueToTFT(2,ADDRESS_MOVE_DISTANCE);
						}
		...
		void DgusTFT::page220_to_223_handle(void)
		{
			static bool  pos_change = false;
			static uint32_t temp_key_value;
			
			if( key_value == 2 || key_value == 4 || key_value == 6 || key_value == 8 || key_value == 10)
			{
					//SERIAL_ECHOLNPAIR_F("axis_homed:",axis_homed);
					//SERIAL_ECHOLNPAIR_F("axis_trusted:",axis_trusted);
					soft_endstop._enabled = false;// Turn off the software limit
					if( !axis_is_trusted(X_AXIS) || !axis_is_trusted(Y_AXIS)){							
							temp_key_value = key_value;
							injectCommands_P("G28");
							queue.process_injected_command_P();
							SysTick_Delay(200);
			 ...
			 switch (key_value) {
				...
				case 1://return
						soft_endstop._enabled = true;// open the software limit
						printer_state = AC_printer_idle;					
						if(pos_change) {
							
							autoProbe.need_save_data = true;
							pos_change = false;
							injectCommands_P("M500");
						}
						ChangePageOfTFT(PAGE_CHS_LEVEING);
						break;
        case 3:   // 0.1f
        ...
        case 2:   // X-
						  if(!isMoving() && axis_is_trusted(X_AXIS) ){
								setAxisPosition_mm(getAxisPosition_mm(X) - move_dis, X, 50);
								autoProbe.calibration_positon.x -= move_dis;
								pos_change = true;
							}
				case 4:   // X+
						  if(!isMoving() && axis_is_trusted(X_AXIS) ){
            ...
            break;
				case 6:   // Y-
						  if(!isMoving() && axis_is_trusted(Y_AXIS) ){
								setAxisPosition_mm(getAxisPosition_mm(Y) - move_dis, Y, 50);
								autoProbe.calibration_positon.y -= move_dis;
								pos_change = true;
							}
            break;
				case 8:   // Y+
            ...
            break;
				case 12:   // save
						...
						autoProbe.postion_data.leveing_postion.x = autoProbe.calibration_positon.x;
						autoProbe.postion_data.leveing_postion.y = autoProbe.calibration_positon.y;
	 ...
	 void DgusTFT::page224_to_225_handle(){
         ...
         if(millis() < (flash_time +1000) )return;
					...
					const char *txt[] = {txt1,txt2,txt3};
					temp = count%3;
					count ++;
					if(count>3) { count = 0;}
					
						
				 switch(autoProbe.LeveingTipStatus){
				
				 case 1:
					SendTxtToTFT(txt[0+temp], TXT_LEVEING_HEATING);
					SendTxtToTFT(TXT_WAIT, TXT_LEVEING_CLEAN);
					SendTxtToTFT(TXT_WAIT, TXT_LEVEING_PROBE);
					break;
				case 2:
					SendTxtToTFT(TXT_OK, TXT_LEVEING_HEATING);
					SendTxtToTFT(txt[0+temp], TXT_LEVEING_CLEAN);
					SendTxtToTFT(TXT_WAIT, TXT_LEVEING_PROBE);
					break;
				 case 3:
				 	SendTxtToTFT(TXT_OK, TXT_LEVEING_HEATING);
					SendTxtToTFT(TXT_OK, TXT_LEVEING_CLEAN);
					SendTxtToTFT(txt[0+temp], TXT_LEVEING_PROBE);
					break;
				 case 4:
				 	ChangePageOfTFT(PAGE_CHS_LEVEING);
				 	break;
		...
		void DgusTFT::page226_to_227_handle(){
				switch (key_value) {
				case 1:
					ChangePageOfTFT(PAGE_CHS_MODULE_CALIBRATION);
					break;		
		...
		void DgusTFT::page228_to_230_handle(){
			char str_buf[20];
			
			switch (key_value) {
				case 1:
					setFeedrate_percent(80);
					recovery.info.print_mode  = 0;
					ChangePageOfTFT(PAGE_CHS_SPEED_SILENT);
					break;
				case 2:
					setFeedrate_percent(100);
					recovery.info.print_mode  = 1;
					ChangePageOfTFT(PAGE_CHS_SPEED_STANDARD);
					break;
				case 3:
					setFeedrate_percent(120);
					recovery.info.print_mode  = 2;
					ChangePageOfTFT(PAGE_CHS_SPEED_SPORT);
					break;
				case 4:
					SendValueToTFT((uint16_t)getFeedrate_percent(), TXT_ADJUST_SPEED); 
					ChangePageOfTFT(PAGE_ADJUST);
					break;
			}
		}
		void DgusTFT::page236_to_237_handle(){
			switch (key_value) {
				case 1:
					injectCommands_P("M502\n M500\n");
					lcd_info.language = ExtUI::ENG;
    				lcd_info.audio = ExtUI::ON;
					ChangePageOfTFT(PAGE_MAIN);
					break;
	...
	void DgusTFT::page_238_to_241_handle(void)
	{
		char str_buf[10];
		float z_off;
		int16_t steps;
...
void DgusTFT::clean_heated_state(){
//		have_heated_task = preheat_state = nozzle_heated_state = bed_heated_state = false;
//		temp_bed_Target = temp_nozzle_Target = 0;
	}
	
	void DgusTFT::Heater_handle(){
		if(!preheat_state) return;
		
		int16_t nozzle_temp_,nozzle_target_temp,bed_temp,bed_target_temp;
		bool nozzle_heating,nozzle_heatedComplete,bed_heating,bed_heatedComplete;
		
		nozzle_temp_ = thermalManager.degHotend(0);
		nozzle_target_temp = thermalManager.degTargetHotend(0);
		bed_temp = thermalManager.degBed();
		bed_target_temp = thermalManager.degTargetBed();
		
		nozzle_heating = nozzle_target_temp > 0? true:false;
		bed_heating = bed_target_temp >0 ? true:false;
		
		nozzle_heatedComplete = ((nozzle_temp_ +5 >= nozzle_target_temp) && nozzle_heating)? true:false;
		bed_heatedComplete = ((bed_temp +5 >= bed_target_temp) && bed_heating)? true:false;

		if(!is_bedHeated && nozzle_heating && nozzle_heatedComplete) {
			preheat_state = false;
			ExtUI::setTargetTemp_celsius(temp_bed_Target, BED);
		}
		else if(is_bedHeated && bed_heating && bed_heatedComplete)
		{
			preheat_state = false;
			is_bedHeated = false;
			ExtUI::setTargetTemp_celsius(temp_nozzle_Target, E0);
		}
...
//		}
	...
	void DgusTFT::SetNozzleOrBedTarget()
	{

		if(!have_heated_task || heated_count == 2) {
			if(heated_count==2) 
			heated_count = 0;
			return;
		}
		...
		int16_t nozzle_temp_,nozzle_target_temp,bed_temp,bed_target_temp;
		static bool nozzle_heating,nozzle_heatedComplete,bed_heating,bed_heatedComplete;
		...
		if(nozzle_heated_state && !bed_heated_state){
			//have_heated_task = false;
			ExtUI::setTargetTemp_celsius(temp_nozzle_Target,E0);
			nozzle_temp_ = thermalManager.degHotend(0);
			nozzle_target_temp = thermalManager.degTargetHotend(0);
		...
		else if(!nozzle_heated_state && bed_heated_state){
			//have_heated_task = false;
			is_bedHeated = true;//先热床加热，加热过程点了喷嘴加热。
			ExtUI::setTargetTemp_celsius(temp_bed_Target,BED);
			bed_temp = thermalManager.degBed();
			bed_target_temp = thermalManager.degTargetBed();
			bed_heatedComplete = ((bed_temp +5 >= bed_target_temp) && bed_heating)? true:false;
		...
		{
			//同时加热到目标温度后,再次加热热床
			if(!bed_heatedComplete && nozzle_heatedComplete) is_bedHeated = true;
			
			//先点热床加热，热床加热过程中再点了喷嘴加热。先热床后喷嘴
			if(is_bedHeated)
			{
				setTargetTemp_celsius(temp_bed_Target,BED);
				//have_heated_task = false;
				preheat_state = true;
			} 
			else
			{	
				//先点喷嘴加热，喷嘴加热过程中再点了热床加热。先加喷嘴后热床
				//同时点了,先喷嘴后热床
				ExtUI::setTargetTemp_celsius(temp_nozzle_Target,E0);
				//have_heated_task = false;
				preheat_state = true;
		...
		if(nozzle_heatedComplete && bed_heatedComplete) {
			nozzle_heated_state = bed_heated_state = false;		
			have_heated_task = false;
		}
  }
} // namespace

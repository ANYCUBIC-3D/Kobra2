...
void idle(TERN_(ADVANCED_PAUSE_FEATURE, bool no_stepper_sleep/*=false*/)) {
  ...
  TERN_(HAS_TFT_LVGL_UI, LV_TASK_HANDLER());
	...
	TERN_(ENDSTOP_SELF_CHECKED,endstop_self_check());

	//if(autoProbe.leveing ||  autoProbe.nedd_open_false == true){
	if(autoProbe.need_openFan == true){
	if(thermalManager.degHotend(0)> 45 && thermalManager.degTargetHotend(0) > 0){
		thermalManager.set_fan_speed(0, 255);
		//autoProbe.leveing = false;
		autoProbe.need_openFan = false;
	}

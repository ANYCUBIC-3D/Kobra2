...
bool endstop_self_check()
{
    if(!thermalManager.allow_cold_extrude) return 0;
    ...
    bool zoffset_state = READ(Z_MAX_PIN)!= Z_MAX_ENDSTOP_INVERTING;
    bool e_state = READ(FIL_RUNOUT_PIN)!= FIL_RUNOUT_STATE;

    static uint8_t cur_endstops_state = e_state << 4 | x_state<<3 | y_state<<2 | z_state<<1 | zoffset_state;
    static uint8_t prv_endstops_state = cur_endstops_state;
                cur_endstops_state = e_state <<4 | x_state<<3 | y_state<<2 | z_state<<1 | zoffset_state;
        
    if(cur_endstops_state == prv_endstops_state) return 0;
            prv_endstops_state = cur_endstops_state;

    if( cur_endstops_state )
            BUZZ(500,3000);
    //        gcode.process_subcommands_now_P("M300 S3000 P500");
    //SERIAL_ECHOLNPAIR("cur_endstops_state:",cur_endstops_state);
}
...
void idle(bool no_stepper_sleep/*=false*/) {
  ...
  watchdog_refresh();//HAL_watchdog_refresh();
  ...
  //Endstop self-checked
  TERN_(IENDSTOP_SELE_CHECRED,endstop_self_check());
...
void kill(FSTR_P const lcd_error/*=nullptr*/, FSTR_P const lcd_component/*=nullptr*/, const bool steppers_off/*=false*/) {
  ...
  const char* error = (const char*)PSTR(lcd_error);
  const char* component = (const char*)PSTR(lcd_component);

  if(HOMING_X == homing_state) {
    homing_state = HOMING_FAILED_X;
    ui.kill_screen(GET_TEXT(MSG_KILL_HOMING_FAILED),"X");
  } else if(HOMING_Y == homing_state) {
    homing_state = HOMING_FAILED_Y;
    ui.kill_screen(GET_TEXT(MSG_KILL_HOMING_FAILED),"Y");
  } else if(HOMING_Z == homing_state) {
    homing_state = HOMING_FAILED_Z;
    ui.kill_screen(GET_TEXT(MSG_KILL_HOMING_FAILED),"Z");
  }
  else ui.kill_screen(error ?: GET_TEXT(MSG_KILLED), component ?: NUL_STR);
...
void setup() {
  ...
  SERIAL_ECHO_MSG(" STEPPER_TIMER_RATE: ",STEPPER_TIMER_RATE);

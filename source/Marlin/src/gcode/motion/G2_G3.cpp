...
#include "../../module/temperature.h"
  ...
  #define MIN_CIRCLE_SEGMENTS 72  // 5бу per segment
#endif
...
void plan_arc(
  ...
  float MIN_ARC_SEGMENT = MIN_ARC_SEGMENT_MM;
  float MAX_ARC_SEGMENT = MAX_ARC_SEGMENT_MM;

  // The number of whole segments in the arc, with best attempt to honor MIN_ARC_SEGMENT_MM and MAX_ARC_SEGMENT_MM
  const uint16_t segments = nominal_segment_mm > (MAX_ARC_SEGMENT) ? CEIL(flat_mm / (MAX_ARC_SEGMENT)) :
                            nominal_segment_mm < (MIN_ARC_SEGMENT) ? _MAX(1, FLOOR(flat_mm / (MIN_ARC_SEGMENT))) :
                            nominal_segments;

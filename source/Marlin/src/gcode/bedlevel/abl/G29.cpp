...
G29_TYPE GcodeSuite::G29() {
  ...
  if (!g29_in_progress) {
    ...
    abl.reenable = planner.leveling_active;
      ...
      if (seen_w) {
        ...
        int8_t i = parser.byteval('I', 0xFF), j = parser.byteval('J', 0xFF);
  ...
  if(probe.status != -1) {
    planner.synchronize();
		...
		#if ENABLED(LEVEING_CALIBRATION_MODULE)
			// autoProbe.write_sdcard_log("G29::autoleveing done");
			// autoProbe.calculation();
			current_position.z -= probe.offset.z;
			sync_plan_position();
			process_subcommands_now("G1 X5 Y5 F3000\n G1 F500 Z0.25\nM500");
		#endif
		autoProbe.LeveingTipStatus = 4;
  } else {
  ...
  // process_subcommands_now("M2003 P");

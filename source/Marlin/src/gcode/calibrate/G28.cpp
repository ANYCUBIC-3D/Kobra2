    ...
    if (position_is_reachable(destination)) {
      ...
      //do_blocking_move_to_xy(destination);
			...
			if(HOMING_FAILED_X != homing_state && HOMING_FAILED_Y != homing_state) {
					...
					if(autoProbe.need_home_leveing)
					{
					...
					else if(autoProbe.need_home_printing){						
//						destination.x = 10;//autoProbe.calibration_positon.x - probe.offset_xy.x;
//						destination.y = 10;//Y_BED_SIZE - probe.offset_xy.y;
//						do_blocking_move_to_xy(destination);
//						thermalManager.set_fan_speed(0, 0);
//						homeaxis(Z_AXIS);

//						if(autoProbe.home_flag == true)
//						{
//							destination.x = autoProbe.home_position.x;
//							destination.y = autoProbe.home_position.y;
//							do_blocking_move_to_xy(destination);
//						}
//						else
//						{
...
//						do_blocking_move_to_z(5,5);
//						thermalManager.set_fan_speed(0, 0);
//						autoProbe.need_home_printing = false;
    ...
    if (z_homing_height && (doX || doY || TERN0(Z_SAFE_HOMING, doZ))) {
      // Raise Z before homing any other axes and z is not already high enough (never lower z)
       if (DEBUGGING(LEVELING)) DEBUG_ECHOLNPAIR("Raise Z (before homing) by ", z_homing_height);
			 do_z_clearance(z_homing_height, false, DISABLED(UNKNOWN_Z_NO_RAISE));

  ...
  void Planner::apply_leveling(xyz_pos_t &raw) {
    if (!leveling_active) return;
      ...
      raw.z += (
          ...
          fade_scaling_factor ? fade_scaling_factor * bilinear_z_offset(raw) : 0.0f
        #endif
      );
  ...
  */
  ...
  if (!block->steps.a && !block->steps.b && !block->steps.c) {
    ...
    accel = CEIL((esteps ? settings.acceleration : settings.travel_acceleration) * steps_per_mm);
  	...
  	if(block->steps.b)
  	{
			if(current_position.z < 250)
			{
				settings.max_acceleration_mm_per_s2[B_AXIS] = -8*current_position.z + 3000;
			}

			else if(250 <current_position.z < 450) 
			{
				settings.max_acceleration_mm_per_s2[B_AXIS] = -4*current_position.z + 2000;
			}

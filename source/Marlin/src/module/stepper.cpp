...
static hal_timer_t start_pulse_count = 0;
...
#define START_TIMED_PULSE() (start_pulse_count = HAL_timer_get_count(PULSE_TIMER_NUM))
#if 0
...
void Stepper::isr() {
  ...
  do {
    ...
    //M4_USART1->DR_f.TDR = PORT_GetBit(PortC,Pin06) + 0x30;
    ...
    const hal_timer_t time_spent = HAL_timer_get_count(STEP_TIMER_NUM);
...
hal_timer_t Stepper::calc_timer_interval(uint32_t step_rate) {
    ...
    uint32_t min_step_rate = uint32_t(STEPPER_TIMER_RATE) / HAL_TIMER_TYPE_MAX;
...
void Stepper::init() {
  ...
  void Stepper::set_shaping_damping_ratio(const AxisEnum axis, const_float_t zeta) {
    ...
    if (zeta <= 0.0f) factor2 = 64.0f;
      ...
      factor2 += 43.073216f * zeta3;
      factor2 = floor(factor2);
    }

    const bool was_on = isr_state();
    isr_off();
  ...
  void Stepper::set_shaping_frequency(const AxisEnum axis, const_float_t freq) {
    ...
    const shaping_time_t delay = freq ? float(uint32_t(STEPPER_TIMER_RATE) / INSHAPPING_TIMER_PRESCALE) / freq : shaping_time_t(-1);
    #if ENABLED(INPUT_SHAPING_X)
      if (axis == X_AXIS) {

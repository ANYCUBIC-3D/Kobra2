...
bool Probe::probe_down_to_z(const float z, const feedRate_t fr_mm_s) {
  ...
  const bool probe_triggered =
      ...
      ||(endstops.trigger_state() & ( _BV(Z_MAX)))
      ;
    #endif
...
float Probe::run_z_probe(const bool sanity_check/*=true*/) {
  ...
  // Double-probing does a fast probe followed by a slow probe
    ...
    if (TERN0(PROBE_TARE, tare())) return NAN;
		thermalManager.set_fan_speed(0, 0);//触发时开启风扇
    // Do a first probe at the fast speed
    #if ENABLED(LEVEING_CALIBRATION_MODULE)
    	//autoProbe.Shake();
	#endif
	...
	thermalManager.set_fan_speed(0, 255);//触发时开启风扇
  ...
  #endif
    {
      ...
      if (TERN0(PROBE_TARE, tare())) return true;
			thermalManager.set_fan_speed(0, 0);//关闭时开启风扇
	#if ENABLED(LEVEING_CALIBRATION_MODULE)
    	//autoProbe.Shake();
	#endif
...
float Probe::probe_at_point(const float &rx, const float &ry, const ProbePtRaise raise_after/*=PROBE_PT_NONE*/, const uint8_t verbose_level/*=0*/, const bool probe_relative/*=true*/, const bool sanity_check/*=true*/) {
  ...
  if (probe_relative) {                                     // The given position is in terms of the probe
	...
	thermalManager.set_fan_speed(0, 255);//开启风扇
  // Move the probe to the starting XYZ
  do_blocking_move_to(npos, feedRate_t(XY_PROBE_FEEDRATE_MM_S));

	thermalManager.set_fan_speed(0, 0);//关闭风扇
	
  float measured_z = NAN;
  if (!deploy()) measured_z = run_z_probe(sanity_check) + offset.z;
  if (!isnan(measured_z)) {
	thermalManager.set_fan_speed(0, 255);//关闭风扇

...
DiskIODriver_SDIO CardReader::sd2card;
#else
DiskIODriver_SPI_SD CardReader::sd2card;
#endif
...
void CardReader::mount() {
  ...
  if (flag.mounted)
    cdroot();
      ...
      if (marlin_state != MF_INITIALIZING) ui.set_status_P(GET_TEXT(MSG_MEDIA_INIT_FAIL), -1);
    #endif
  }
...
void CardReader::endFilePrintNow(TERN_(SD_RESORT, const bool re_sort/*=false*/)) {
  uint8_t temp_did_pause_print = did_pause_print;
  ...
  if(temp_did_pause_print) flag.abort_sd_printing = true;
}

...
class SdVolume {
 public:
   ...
   * floppy format.
   *
   * \param[in] dev The DiskIODriver_SDIO where the volume is located.
   *
   * \return true for success, false for failure.
   * Reasons for failure include not finding a valid partition, not finding
   * a valid FAT file system or an I/O error.
   */
  bool init(DiskIODriver_SDIO *dev) { return init(dev, 1) || init(dev, 0); }
  bool init(DiskIODriver_SDIO *dev, uint8_t part);
  ...
  /**
   * DiskIODriver_SDIO object for this volume
   * \return pointer to DiskIODriver_SDIO object.
   */
  DiskIODriver_SDIO* sdCard() { return sdCard_; }
 ...
 private:
  ...
  static bool const CACHE_FOR_WRITE = true;
    ...
    DiskIODriver_SDIO *sdCard_;       // DiskIODriver_SDIO object for cache
    bool cacheDirty_;            // cacheFlush() will write block if true
    uint32_t cacheMirrorBlock_;  // block number for mirror FAT
  #else
    static cache_t cacheBuffer_;        // 512 byte cache for device blocks
    static uint32_t cacheBlockNumber_;  // Logical number of block in the cache
    static DiskIODriver_SDIO *sdCard_;       // DiskIODriver_SDIO object for cache

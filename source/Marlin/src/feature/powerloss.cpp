...
const char PrintJobRecovery::filename[20] = "/Log.txt";
...
//	SERIAL_ECHOLNPAIR("head=", info.valid_head,"foot=", info.valid_foot);
    ...
    write();
		printf("info.z: %f %f\n", info.current_position.z,temp_position.z);
  }
}
    ...
    void PrintJobRecovery::outage() {
	  ...
	  if(current_position.z < 0.5) return;
  ...
  //    __disable_irq();
    ...
    SERIAL_ECHOLNPAIR("x:",current_position.x,"y:",current_position.y);
...
void PrintJobRecovery::resume() {
  ...
  const uint32_t resume_sdpos = info.sdpos; // Get here before the stepper ISR overwrites it
   ...
   gcode.process_subcommands_now_P(PSTR("M413 S0"));
  ...
  // Reset E, raise Z, home XY...
    ...
    sprintf_P(cmd, PSTR("G92.9 E0 " TERN(BACKUP_POWER_SUPPLY, "Z%s", "Z0\nG1Z%s")), dtostrf(info.zraise, 1, 3, str_1));
		//sprintf_P(cmd, PSTR("G92.9 E0 Z%.3f"),info.zraise);
		//printf(cmd);
		gcode.process_subcommands_now(cmd);
  ...
  gcode.process_subcommands_now("M114 D");
#endif
  SERIAL_ECHOLNPAIR("rx:",current_position.x,"ry:",current_position.y);
...
// Move back to the saved Z
  ...
  enable(true);
  gcode.process_subcommands_now_P(PSTR("M413 S1"));

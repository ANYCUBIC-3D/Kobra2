...
void PrintJobRecovery::save(const bool force/*=false*/, const float zraise/*=POWER_LOSS_ZRAISE*/, const bool raised/*=false*/) {
  ...
  // We don't check IS_SD_PRINTING here so a save may occur during a pause
    ...
    #define POWER_LOSS_MIN_Z_CHANGE 0.05f  // Vase-mode-friendly out of the box
  #endif
    float LOSS_MIN_Z_CHANGE = POWER_LOSS_MIN_Z_CHANGE;
  ...
  if (force
      ...
      || current_position.z > info.current_position.z + LOSS_MIN_Z_CHANGE
    #endif
  ) {
    ...
    printf("info.z0: %f %f\n", info.current_position.z , current_position.z);
    ...
    write();
    printf("info: x:%f y:%f z:%f\n", info.current_position.x , info.current_position.y ,info.current_position.z);
...
void PrintJobRecovery::resume() {
  ...
  gcode.process_subcommands_now(F("G92.9E0")); // Reset E to 0
    ...
    float z_now = info.flag.raised ? z_raised : z_print;
      ...
      //sprintf_P(cmd, PSTR("G92.9Z%s"), dtostrf(z_now, 1, 3, str_1));
			sprintf_P(cmd, PSTR("G92.9 Z%s"), dtostrf(0, 1, 3, str_1));
			gcode.process_subcommands_now(cmd);
    #endif
    ...
    gcode.process_subcommands_now(F("G28 R0 XY")); // No raise during G28
  #endif
  ...
  set_all_homed();
    ...
    gcode.process_subcommands_now(cmd);
      ...
      SERIAL_ECHOLNPAIR("current_position.z:",temp_current_position.z,"z_now:",z_now,"z_raised",z_raised,"z_print", z_print);
 ...
 // Move back over to the saved XY
  SERIAL_ECHOLNPGM("x:",temp_current_position.x,"y:",temp_current_position.y);
  ...
  sprintf_P(cmd, PSTR("G1 Z%s F600"), dtostrf(z_print, 1, 3, str_1));
  gcode.process_subcommands_now(cmd);
...
// /*************************************/
  ...
  print_job_timer.power_outage_resume();

...
bool load_filament(const_float_t slow_load_length/*=0*/, const_float_t fast_load_length/*=0*/, const_float_t purge_length/*=0*/, const int8_t max_beep_count/*=0*/,
  ...
  if (pause_for_user) {
    ...
    TERN_(EXTENSIBLE_UI, ExtUI::onUserConfirmRequired(("Load Filament")));
...
bool pause_print(const_float_t retract, const xyz_pos_t &park_point, const bool show_lcd/*=false*/, const_float_t unload_length/*=0*/ DXC_ARGS) {
  ...
  TERN_(DWIN_LCD_PROUI, if (!do_park) ui.set_status_P(GET_TEXT(MSG_PARK_FAILED)));
...
void wait_for_confirmation(const bool is_reload/*=false*/, const int8_t max_beep_count/*=0*/ DXC_ARGS) {
  ...
  TERN_(EXTENSIBLE_UI, ExtUI::onUserConfirmRequired(GET_TEXT(MSG_NOZZLE_PARKED)));
  wait_for_user = true;    // LCD click or M108 will clear this
  while (wait_for_user) {
    impatient_beep(max_beep_count);
    ...
    if (nozzle_timed_out) {
      ...
      TERN_(EXTENSIBLE_UI, ExtUI::onUserConfirmRequired(GET_TEXT(MSG_HEATER_TIMEOUT)));

      TERN_(HAS_RESUME_CONTINUE, wait_for_user_response(0, true)); // Wait for LCD click or M108

      TERN_(HOST_PROMPT_SUPPORT, hostui.prompt_do(PROMPT_INFO, GET_TEXT_F(MSG_REHEATING)));

      TERN_(EXTENSIBLE_UI, ExtUI::onStatusChanged(GET_TEXT(MSG_REHEATING)));
      ...
      TERN_(EXTENSIBLE_UI, ExtUI::onUserConfirmRequired(GET_TEXT(MSG_REHEATDONE)));
...
void resume_print(const_float_t slow_load_length/*=0*/, const_float_t fast_load_length/*=0*/, const_float_t purge_length/*=ADVANCED_PAUSE_PURGE_LENGTH*/, const int8_t max_beep_count/*=0*/, const celsius_t targetTemp/*=0*/ DXC_ARGS) {
  ...
  */
  ...
  load_filament(slow_load_length, fast_load_length, purge_length, max_beep_count, false, nozzle_timed_out, PAUSE_MODE_SAME DXC_PASS);

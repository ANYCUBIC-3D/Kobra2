...
void fan_pwm_set_ratio(uint8_t fan, uint8_t ratio)
{
    ...
    if(is_onff_fan){
		if(fan == 0){
            if(ratio)
            {	
                PORT_SetBits(BOARD_PWM_CH0_PORT, BOARD_PWM_CH0_PIN);
            }
            else 
            {
                PORT_ResetBits(BOARD_PWM_CH0_PORT, BOARD_PWM_CH0_PIN);
            }
        }
        return;
    }
...
//    periodValue = (uint32_t)(100000000ul / (1 << M4_TMRA3->BCSTR_f.CKDIV)) / frequency;
   periodValue = (uint32_t)(84000000ul / (1 << M4_TMRA3->BCSTR_f.CKDIV)) / frequency;
    // periodValue = (uint32_t)(64000000ul / (1 << M4_TMRA3->BCSTR_f.CKDIV)) / frequency;
...
void fan_set_onoff_init(){
	...
	PORT_SetFunc(BOARD_PWM_CH0_PORT,BOARD_PWM_CH0_PIN,Func_Gpio,Disable);
	PORT_Init(BOARD_PWM_CH0_PORT, BOARD_PWM_CH0_PIN, &stcPortInit);
	is_onff_fan = true;
}

void fan_set_on_off(uint8_t fan,bool state){

	if(fan == 0){
		
			if(state)
			{	
				PORT_SetBits(BOARD_PWM_CH0_PORT, BOARD_PWM_CH0_PIN);//open fan		
			}
			else  
			{
				PORT_ResetBits(BOARD_PWM_CH0_PORT, BOARD_PWM_CH0_PIN);//close fan
			}
	}
